...
- The version string is duplicated in **four** places that must stay in sync: `manifest.json:4` (`version`), `README.md` ("Versión" section), `popup.js:437` and `popup.html:1253` (both the same footer fallback text, `"v2.X · BCP Target Inspector"`, shown when `instanceInfo` hasn't loaded yet). Same trap as the `ALLOWED_DOMAINS` duplication above — bump only the manifest and the UI footer keeps showing the old version.
